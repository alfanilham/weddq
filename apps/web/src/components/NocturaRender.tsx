...
/* Noctura — Modern dark luxe.
   Near-black canvas, off-white type, animated gold shimmer on the couple names,
   floating gold particles, and hairline gold accents. Exclusive tier. */
...
const PARTICLES = [
  { l: "8%", t: "18%", s: 3, d: 0 }, { l: "22%", t: "62%", s: 2, d: 1.4 }, { l: "40%", t: "30%", s: 4, d: 2.1 },
  { l: "63%", t: "70%", s: 2, d: 0.7 }, { l: "78%", t: "22%", s: 3, d: 1.9 }, { l: "90%", t: "55%", s: 2, d: 2.8 },
  { l: "15%", t: "84%", s: 2, d: 3.2 }, { l: "55%", t: "12%", s: 3, d: 1.1 },
];
function Particles() {
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none" aria-hidden>
      {PARTICLES.map((p, i) => (
        <span key={i} className="noc-particle absolute rounded-full" style={{ left: p.l, top: p.t, width: p.s, height: p.s, background: TONE.goldSoft, animationDelay: `${p.d}s` }} />
      ))}
    </div>
  );
}

function Mono({ a, b, color = TONE.gold, size = 60 }: { a: string; b: string; color?: string; size?: number }) {
  return (
    <div className="inline-flex items-center justify-center" style={{ width: size, height: size }}>
      <div className="relative flex items-center justify-center w-full h-full" style={{ border: `1px solid ${color}55`, borderRadius: 4, transform: "rotate(45deg)" }}>
        <span className="font-serif italic" style={{ color, fontSize: size * 0.36, transform: "rotate(-45deg)" }}>{a[0]}&amp;{b[0]}</span>
      </div>
    </div>
  );
}
...
function Eyebrow({ children, color = TONE.gold }: { children: ReactNode; color?: string }) {
  return (
    <div className="flex items-center justify-center gap-3">
      <span className="h-px w-10" style={{ background: `linear-gradient(90deg, transparent, ${color})` }} />
      <span className="text-[10px] tracking-[0.45em] uppercase font-medium" style={{ color }}>{children}</span>
      <span className="h-px w-10" style={{ background: `linear-gradient(90deg, ${color}, transparent)` }} />
    </div>
  );
}
...
export function NocturaRender({ data, interactive = false }: { data: InvitationData; interactive?: boolean }) {
  ...
  return (
    <div style={{ background: TONE.bg, color: TONE.ink }} className="font-sans">
      <style>{`
        @keyframes noc-shine { to { background-position: 200% center; } }
        @keyframes noc-drift { 0%,100%{ transform: translateY(0); opacity:.5 } 50%{ transform: translateY(-14px); opacity:1 } }
        .noc-shimmer{ background: linear-gradient(100deg, ${TONE.goldSoft} 0%, #FFF6DC 18%, ${TONE.gold} 38%, ${TONE.goldSoft} 58%, #FFF6DC 78%, ${TONE.gold} 100%); background-size:200% auto; -webkit-background-clip:text; background-clip:text; color:transparent; animation: noc-shine 6s linear infinite; }
        .noc-particle{ animation: noc-drift 7s ease-in-out infinite; }
        .noc-input{ width:100%; border:1px solid ${TONE.line}; background:${TONE.panel2}; padding:12px 14px; font-size:14px; color:${TONE.ink}; border-radius:10px; font-family:inherit }
        .noc-input::placeholder{ color:${TONE.inkSoft} }
        .noc-input:focus{ outline:none; border-color:${TONE.gold} }
      `}</style>
      {!opened ? (
        ...
        <div className="weddq-reveal">
          ...
          {((data.storyChapters && data.storyChapters.length > 0) || data.story) && (
            <StoryChapters chapters={data.storyChapters} story={data.story} gallery={data.gallery}
              theme={{ bg: TONE.panel, fg: TONE.ink, fgSoft: TONE.inkSoft, accent: TONE.gold, rule: TONE.gold, card: TONE.panel2, variant: "dark" }} />
          )}
...
function Cover({ data, onOpen }: { data: InvitationData; onOpen: () => void }) {
  const cover = data.coverImage || data.couple.bridePhoto || "https://images.unsplash.com/photo-1511285560929-80b456fea0bc?w=1600&q=80";
  ...
  return (
    <section className="relative min-h-[100svh] flex items-center justify-center px-5 py-12 overflow-hidden">
      ...
      <div className="absolute inset-0" style={{ background: `radial-gradient(circle at 50% 40%, ${TONE.bg}66 0%, ${TONE.bg}cc 55%, ${TONE.bg}f7 100%)` }} />
      <Particles />
      <div className="relative w-full max-w-xl mx-auto text-center" style={{ textShadow: "0 2px 16px rgba(0,0,0,0.6), 0 1px 2px rgba(0,0,0,0.6)" }}>
        <Mono a={data.couple.brideShort} b={data.couple.groomShort} color={TONE.goldSoft} size={64} />
        <div className="mt-7"><Eyebrow color={TONE.goldSoft}>{data.eyebrow}</Eyebrow></div>
        <h1 className="font-serif mt-7 noc-shimmer" style={{ fontSize: "clamp(44px, 9.5vw, 76px)", lineHeight: 1.02, letterSpacing: "0.01em", textShadow: "none" }}>
          {data.couple.brideShort}
          <span className="block font-serif italic my-1" style={{ fontSize: "0.4em" }}>&amp;</span>
          {data.couple.groomShort}
        </h1>
        <div className="mt-7 flex items-center justify-center gap-3 text-[11px] tracking-[0.4em] uppercase" style={{ color: `${TONE.ink}cc` }}>
        ...
        {guestName && (
          <div className="mt-8 flex justify-center">
            <div className="px-6 py-3.5 rounded-xl" style={{ background: "rgba(255,255,255,0.05)", border: `1px solid ${TONE.gold}55`, backdropFilter: "blur(6px)" }}>
              <div className="text-[10px] tracking-[0.32em] uppercase" style={{ color: TONE.goldSoft }}>Kepada Yang Terhormat</div>
              <div className="font-serif text-xl mt-1" style={{ color: TONE.ink }}>{guestName}</div>
            </div>
          </div>
        )}
        <div className="mt-9 flex justify-center">
          <button onClick={onOpen} className="group inline-flex items-center gap-3 rounded-full px-8 py-3.5 text-[11px] tracking-[0.35em] uppercase transition hover:scale-105" style={{ background: `linear-gradient(100deg, ${TONE.gold}, ${TONE.goldSoft})`, color: TONE.bg }}>
...
function CoupleCard({ role, person }: { role: string; person: { name: string; short: string; parents?: string | null; ig?: string | null; photo?: string | null } }) {
  return (
    <article className="p-8 text-center" style={{ background: TONE.panel, borderRadius: 20, border: `1px solid ${TONE.line}` }}>
      <div className="relative mx-auto w-[150px] h-[150px] overflow-hidden rounded-full" style={{ border: `1px solid ${TONE.gold}66`, boxShadow: `0 0 0 6px rgba(201,162,75,0.08)` }}>
        {person.photo ? <img src={person.photo} alt={person.name} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center font-serif text-5xl" style={{ background: TONE.panel2, color: TONE.gold }}>{person.short[0]}</div>}
      </div>
...
function DateMoment({ primary, couple }: { primary: string; couple: InvitationData["couple"] }) {
  ...
  return (
    <Section bg={TONE.panel} className="text-center overflow-hidden">
      <Particles />
      <Reveal className="relative">
        <Mono a={couple.brideShort} b={couple.groomShort} color={TONE.goldSoft} size={54} />
        <div className="mt-5"><Eyebrow color={TONE.goldSoft}>Menuju Hari Bahagia</Eyebrow></div>
        <div className="font-serif mt-6 noc-shimmer" style={{ fontSize: "clamp(76px, 14vw, 132px)", lineHeight: 1 }}>{dp.day}</div>
        <div className="font-serif italic mt-2" style={{ color: TONE.ink, fontSize: 22 }}>{dp.month} {dp.year}</div>
        ...
        <div className="mt-10 grid grid-cols-4 gap-3 max-w-md mx-auto">
          {[["Hari", cd.d], ["Jam", cd.h], ["Menit", cd.m], ["Detik", cd.s]].map(([l, v]) => (
            <div key={l as string} className="text-center py-4 px-2 rounded-xl" style={{ background: TONE.panel2, border: `1px solid ${TONE.line}` }}>
              <div className="font-serif text-2xl md:text-3xl tabular-nums" style={{ color: TONE.ink }}>{String(v).padStart(2, "0")}</div>
        ...
        {events.map((e, i) => {
          ...
          return (
            <Reveal key={e.id ?? i} delay={i * 80}>
              <article className="p-7 grid md:grid-cols-[auto_1fr] gap-6 items-start" style={{ background: TONE.panel, borderRadius: 18, border: `1px solid ${TONE.line}` }}>
                <div className="text-center px-5 py-4 self-center rounded-xl" style={{ background: TONE.panel2, color: TONE.ink, border: `1px solid ${TONE.line}` }}>
                ...
                <div>
                  ...
                  <div className="mt-4 flex flex-wrap items-center gap-3">
                    {e.dressCode && <span className="text-[10px] tracking-[0.22em] uppercase px-3 py-1 rounded-full" style={{ background: TONE.panel2, color: TONE.inkSoft }}>Dresscode: {e.dressCode}</span>}
                    {e.mapUrl && <a href={e.mapUrl} target="_blank" rel="noreferrer" className="text-[10px] tracking-[0.22em] uppercase px-4 py-1.5 inline-flex items-center gap-1.5 rounded-full" style={{ background: `linear-gradient(100deg, ${TONE.gold}, ${TONE.goldSoft})`, color: TONE.bg }}>Lihat Peta <span>→</span></a>}
                  </div>
                </div>
              </article>
            </Reveal>
...
function Gallery({ gallery }: { gallery: InvitationData["gallery"] }) {
  ...
  return (
    <Section bg={TONE.panel}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        {gallery.map((g, i) => (
          <Reveal key={g.id ?? i} delay={(i % 3) * 70}>
            <button onClick={() => setLightbox(g.url)} className="relative overflow-hidden group aspect-[3/4] w-full rounded-xl" style={{ border: `1px solid ${TONE.line}`, background: TONE.panel2 }}>
      ...
      {lightbox && <div onClick={() => setLightbox(null)} className="fixed inset-0 z-50 flex items-center justify-center p-6 cursor-zoom-out" style={{ background: "rgba(8,8,10,0.96)" }}><img src={lightbox} alt="" className="max-w-full max-h-full object-contain rounded-xl" /></div>}
    </Section>
  );
}
...
function Rsvp({ data, interactive }: { data: InvitationData; interactive: boolean }) {
  ...
  return (
    <Section bg={TONE.bg} className="text-center">
      <Reveal>
        <Eyebrow color={TONE.goldSoft}>Konfirmasi Kehadiran</Eyebrow>
        <Title>RSVP</Title>
        <p className="mt-5 text-sm leading-relaxed max-w-md mx-auto mb-8" style={{ color: TONE.inkSoft }}>
          {known ? <>Halo <span className="italic font-medium" style={{ color: TONE.goldSoft }}>{data.guestName}</span>, mohon konfirmasikan kehadiran Anda.</> : "Mohon konfirmasikan kehadiran Anda agar kami dapat menyiapkan tempat sebaik-baiknya."}
        </p>
        {done ? (
          <div className="max-w-md mx-auto p-9 rounded-2xl" style={{ background: TONE.panel, border: `1px solid ${TONE.line}` }}>
            <div className="font-serif text-2xl" style={{ color: TONE.goldSoft }}>Terima kasih.</div>
            <p className="text-sm mt-2" style={{ color: TONE.inkSoft }}>Konfirmasi Anda telah kami terima.</p>
          </div>
        ) : (
          <form onSubmit={submit} className="grid gap-4 max-w-md mx-auto p-7 text-left rounded-2xl" style={{ background: TONE.panel, border: `1px solid ${TONE.line}` }}>
            {!known && <Field label="Nama lengkap"><input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="noc-input" /></Field>}
            {known ? (
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {STATUS.map((s) => { const active = form.status === s.id; return (
                    <button key={s.id} type="button" onClick={() => setForm({ ...form, status: s.id })} className="text-xs py-3 rounded-lg transition" style={{ border: `1px solid ${active ? TONE.gold : TONE.line}`, background: active ? `linear-gradient(100deg, ${TONE.gold}, ${TONE.goldSoft})` : "transparent", color: active ? TONE.bg : TONE.ink }}>{s.label}</button>
                  ); })}
                </div>
              </div>
            ) : (
              <div className="grid grid-cols-2 gap-4">
                <Field label="Kehadiran"><select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })} className="noc-input"><option value="HADIR">Hadir</option><option value="TIDAK">Tidak Hadir</option><option value="RAGU">Masih Ragu</option></select></Field>
                <Field label="Jumlah tamu"><input type="number" min={1} max={6} value={form.pax} onChange={(e) => setForm({ ...form, pax: Number(e.target.value) })} className="noc-input" /></Field>
              </div>
            )}
            <Field label="Pesan untuk Mempelai (opsional)"><textarea value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} rows={3} className="noc-input" /></Field>
            {err && <div className="text-sm" style={{ color: "#e0928a" }}>{err}</div>}
            <button type="submit" disabled={busy} className="py-3 rounded-full text-[11px] tracking-[0.32em] uppercase mt-1" style={{ background: `linear-gradient(100deg, ${TONE.gold}, ${TONE.goldSoft})`, color: TONE.bg }}>{busy ? "Mengirim…" : "Kirim Konfirmasi"}</button>
          </form>
        )}
      </Reveal>
    </Section>
...
function Wishes({ slug, initial, interactive }: { slug: string; initial: NonNullable<InvitationData["wishes"]>; interactive: boolean }) {
  ...
  return (
    <Section bg={TONE.panel}>
      ...
      <form onSubmit={submit} className="grid gap-3 mb-8 p-6 max-w-md mx-auto rounded-2xl" style={{ background: TONE.panel2, border: `1px solid ${TONE.line}` }}>
        <input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Nama Anda" className="noc-input" />
        <textarea required rows={3} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} placeholder="Tuliskan doa & ucapan terbaik Anda…" className="noc-input" />
        <button disabled={busy} className="py-3 rounded-full text-[11px] tracking-[0.32em] uppercase" style={{ background: `linear-gradient(100deg, ${TONE.gold}, ${TONE.goldSoft})`, color: TONE.bg }}>{busy ? "Mengirim…" : "Kirim Ucapan"}</button>
      </form>
      {list.length === 0 ? (
        ...
        <div className="grid md:grid-cols-2 gap-3 max-h-[500px] overflow-y-auto pr-2">
          {list.map((w, i) => (
            <article key={w.id ?? i} className="p-5 rounded-xl" style={{ background: TONE.panel2, border: `1px solid ${TONE.line}` }}>
...
function GiftCard({ g }: { g: { kind: string; bankName: string; number: string; holder: string } }) {
  ...
  return (
    <div className="relative p-6 rounded-2xl h-full overflow-hidden" style={{ background: TONE.panel, border: `1px solid ${TONE.gold}44` }}>
      <div className="absolute -top-10 -right-8 opacity-10 font-serif" style={{ color: TONE.gold, fontSize: 150, lineHeight: 1 }}>✦</div>
      <div className="relative">
        ...
        <button onClick={() => navigator.clipboard.writeText(g.number).then(() => { setCopied(true); setTimeout(() => setCopied(false), 1500); })} className="mt-3 text-[10px] tracking-[0.25em] uppercase px-3 py-1.5 rounded-full" style={{ background: `linear-gradient(100deg, ${TONE.gold}, ${TONE.goldSoft})`, color: TONE.bg }}>{copied ? "Tersalin ✓" : "Salin Nomor"}</button>
      </div>
    </div>
  );
}
...
function Closing({ data }: { data: InvitationData }) {
  ...
  return (
    <Section bg={TONE.panel} className="text-center overflow-hidden">
      <Particles />
      <Reveal className="relative">
        <Mono a={data.couple.brideShort} b={data.couple.groomShort} color={TONE.goldSoft} size={58} />
        <div className="mt-5"><Eyebrow color={TONE.goldSoft}>Salam Penutup</Eyebrow></div>
        <p className="font-serif italic mt-6 max-w-2xl mx-auto" style={{ color: TONE.ink, fontSize: "clamp(20px, 3vw, 28px)", lineHeight: 1.55 }}>Merupakan suatu kehormatan apabila Bapak/Ibu/Saudara/i berkenan hadir untuk memberikan doa restu.</p>
        ...
        <div className="font-serif mt-4 noc-shimmer" style={{ fontSize: "clamp(40px, 8vw, 72px)", lineHeight: 1 }}>{data.couple.brideShort} &amp; {data.couple.groomShort}</div>
